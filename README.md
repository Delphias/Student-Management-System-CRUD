...
•	Run command to start the backend  npm start or npm run dev
...
6.	The frontend runs on http://localhost:5173 by default.
API requests are proxied to the backend (http://localhost:8000) via vite.config.ts

## Tech Stack
• 	MongoDB Atlas → Cloud database for student records
• 	Express.js → Backend REST API framework
• 	React.js (Vite) → Frontend UI with hooks
• 	Node.js → Runtime environment for backend
• 	Tailwind CSS → Styling and responsive design

## Challenges Faced: MongoDB Atlas Error
•	Connection errors with MongoDB Atlas when using SRV connection strings.
•	Backend couldn’t connect when cluster IP wasn’t whitelisted.
•	DNS resolution issues caused ECONNREFUSED errors.
